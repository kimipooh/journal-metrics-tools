...
>
> **v1.0.0 注記**: 実装済み adapter は `mock` / `sealib` / `sinta` のみ。旧 `metrics_excel.py` は historical implementation として再構築時に参照されたが、v1.0.0 前に削除済みであり、現行実装は `journal_metrics.py` / `journal_mapper.py` / `adapters/` である。
...
- `metric_source` を3つの役割に分類する: **metrics source**（`SINTA`、Program2投入対象、grade必須）/ **reference source**（`SEALIB`、投入対象外）/ **test source**（`MOCK`、投入対象外）。
...
| --- | --- | --- | --- | --- |
| **metrics source** | `SINTA` | adapterが必ず設定する想定 | ○ | 本番指標投入（journal_metrics REST API公開対象） |
...
`journal_metrics` へ投入する実指標を提供するadapter。`grade` / `url` / `note` 等が実データとして埋まることが前提。v1.0.0 時点では `sinta` adapter が該当する。実装時はgradeを必ず設定する契約とする（adapter contract側の要件は別タスクで整理）。
...
- SINTA adapter の追加拡張
- grade値の正規化（historical implementation である旧 `metrics_excel.py` の正規化ロジックを参考にするかは別途判断）
