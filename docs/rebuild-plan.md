...
>
> **v1.0.0 注記**: 旧 `metrics_excel.py` は再構築時の historical reference として参照されたが、v1.0.0 前に削除済みであり、リリース対象には含めない。現行実装は `journal_metrics.py` / `journal_mapper.py` / `adapters/` による 3 シート workbook workflow である。

## 0. 方針サマリ

- **段階的移行ではなく、完全再設計・再実装**を行う。
- 旧 `metrics_excel.py`（SEALIB/SINTA 2 シート方式）は **legacy reference として参照された historical implementation** であり、v1.0.0 には含めない。
...
- 旧 `metrics_excel.py` は再構築時に **legacy reference** として扱った historical implementation であり、v1.0.0 前に削除済み。
- v1.0.0 の現行 CLI は `journal_metrics.py`。`journal_mapper.py` と `adapters/` を組み合わせた新 3 シート workbook workflow を正式実装とする。
- ドキュメント上で `metrics_excel.py` と記す場合は、削除済み旧実装・旧運用・historical reference の説明に限定する。

## 1. 背景

本ツールは独立した**汎用 Journal Metrics 収集・確認・変換ツール**を目指す。SEALIB は利用先の一つに過ぎず、SINTA や将来の他ソースを扱う。
...
- 旧 `metrics_excel.py` は再構築時の設計参考としてのみ使用した。
- v1.0.0 では旧 `metrics_excel.py` を同梱しない。
- 現行実装は `journal_metrics.py` / `journal_mapper.py` / `adapters/` による 3 シート workbook workflow とする。
- README / CHANGELOG は現行 workflow のみを説明し、旧ツールを実行可能な選択肢として案内しない。
...
| **grade 正規化** | `normalized_grade()`（`export_command` `:617` で使用、`(grade, warning)` を返す。例: `S1` → `S1 Accredited`、未知値は警告） | historical reference として必要に応じて参照。現行 v1.0.0 では adapter が返す SINTA grade をそのまま扱う |
...
```
main (人が編集する入力フォーム)
   │  fetch-journal（外部アダプタ: SINTA / 将来ソース / …）
   ▼
...
- 互換用の旧コマンドは新ツールに実装しない。旧 `metrics_excel.py` は v1.0.0 に含めない。
...
- 旧 `metrics_excel.py` は legacy reference として参照のみ行い、直接改造しない。
...
- `convert_status` は source role と grade の有無で決定する。`SINTA` かつ grade 非空のみ `ready`、grade空は `hold`、`SEALIB` / `MOCK` / 未知sourceは `skipped`。
...
運用上は `main.name` を SEALIB DB の正式 `header.name` に合わせることが重要である。`search_query` は SINTA 等の外部 adapter 用検索語であり、Program2 の name matching には使わない。`name mismatch; resolved by sealib_id fallback` warning は、実装問題ではなく、`main.name` と DB の正式 `header.name` の不一致・古い名称・手入力誤差で発生し得る。
...
- **fetch adapter（外部 CLI）**: 入力＝検索キー（`main.search_query`、空なら移行補助として `main.journal_name`。`main.name` には fallback しない）→ 出力＝候補配列（JSON）。各候補を journal 行へ写像。`journal_type` は呼び出し側が付与し、`grade` 正規化と `raw_json` 保存はツール側で行う。SINTA や将来ソースはアダプタ差し替えで対応。
...
- **旧コマンド互換は提供しない**（破壊的）。旧 2 シート Excel 用の `metrics_excel.py` は historical reference であり、v1.0.0 には含めない。
...
2. 旧 `metrics_excel.py` は legacy reference として扱い、拡張しない。v1.0.0 前に削除する。
3. **adapter contract（§7）を最初に固める**ことで SINTA や将来ソースを差し替え可能にする。
